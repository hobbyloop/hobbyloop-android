...
import com.hobbyloop.core.ui.componenet.datePicker.util.SelectorOptions
    ...
    Box(
        ...
        LazyColumn(
            ...
            items(if (isEndless) Int.MAX_VALUE else count) {
                ...
                val scale = calculateScale(focusedIndex.value, it, rowOffsetCount)
                Box(
                    modifier = Modifier
                        ...
                        .graphicsLayer {
                            ...
                            this.alpha = if (focusedIndex.value == it) 1f else 0.5f
                            this.scaleX = scale
                            this.scaleY = scale
                        },
        ...
        if (selectorOption.enabled) {
            SelectionView(
                modifier.height(size.height).fillMaxWidth(), selectorOptions = selectorOption, rowOffset
            )
        }
...
private fun calculateScale(focusedIndex: Int, index: Int, maxOffset: Int): Float {
    val distance = abs(focusedIndex - index)
    return if (distance <= maxOffset) {
        1f - (distance.toFloat() / (maxOffset * 2))
    } else {
        0.5f
    }
}
